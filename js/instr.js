class Instructions {
    ...
    saveReadingTime() {
        if (typeof(this.readingTimes[this.index])=='undefined'){ //will record the reading duration for the first time
            this.readingTimes[this.index] = this.readingDuration;
        }
        else{
            if (this.readingTimes[this.index] < this.readingDuration){ //only update reading time if it's greater then the previous record
                this.readingTimes[this.index] = this.readingDuration;
            }
        }
    }
